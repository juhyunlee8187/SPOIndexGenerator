    ...
    static async Task Main(string[] args)
    {
        ...
        try
        {
            ...
            if (indexDataChoice.ToLower() == "y")
            {
                ...
                var jsonContent = await graphUsingHttp.GetSPOFilesAsync(appConfig.SiteId, appConfig.DriveId, authenticationResult.AccessToken);
                if (jsonContent == null)
                {
                ...
                foreach (var item in jsonContent)
                {
                    ...
                    List<DocumentIndex> documentIndexes = await azureSearchIndexHelper.GenerateDocumentIndexDateAsync(paragraphs, itemValue);
